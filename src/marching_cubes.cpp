...
void MarchingCubes::setupVBOs() {
  ...
  for (int i = 0; i < nx - 1; i++) {
    for (int j = 0; j < ny - 1; j++) {
      for (int k = 0; k < nz - 1; k++) {
        ...
        v[0] = GridValue(glm::vec3(dx * i + eps, dy * j + eps, dz * k + eps),
                         getNormal(i, j, k), get(i, j, k));
        v[1] =
            GridValue(glm::vec3(dx * i + eps, dy * j + eps, dz * (k + 1) - eps),
                      getNormal(i, j, k + 1), get(i, j, k + 1));
        v[2] =
            GridValue(glm::vec3(dx * i + eps, dy * (j + 1) - eps, dz * k + eps),
                      getNormal(i, j + 1, k), get(i, j + 1, k));
        v[3] = GridValue(
            glm::vec3(dx * i + eps, dy * (j + 1) - eps, dz * (k + 1) - eps),
            getNormal(i, j + 1, k + 1), get(i, j + 1, k + 1));
        v[4] =
            GridValue(glm::vec3(dx * (i + 1) - eps, dy * j + eps, dz * k + eps),
                      getNormal(i + 1, j, k), get(i + 1, j, k));
        v[5] = GridValue(
            glm::vec3(dx * (i + 1) - eps, dy * j + eps, dz * (k + 1) - eps),
            getNormal(i + 1, j, k + 1), get(i + 1, j, k + 1));
        v[6] = GridValue(
            glm::vec3(dx * (i + 1) - eps, dy * (j + 1) - eps, dz * k + eps),
            getNormal(i + 1, j + 1, k), get(i + 1, j + 1, k));
        v[7] =
            GridValue(glm::vec3(dx * (i + 1) - eps, dy * (j + 1) - eps,
                                dz * (k + 1) - eps),
                      getNormal(i + 1, j + 1, k + 1), get(i + 1, j + 1, k + 1));
        // need to alternate orientation of central tetrahedron to ensure that
  ...
  glBufferData(GL_ARRAY_BUFFER,
               sizeof(VBOPosNormalColor) * marching_cubes_verts.size(),
               &marching_cubes_verts[0], GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, marching_cubes_tri_indices_VBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               sizeof(VBOIndexedTri) * marching_cubes_tri_indices.size(),
               &marching_cubes_tri_indices[0], GL_STATIC_DRAW);
}
...
void MarchingCubes::drawVBOs() {
  ...
  glDrawElements(GL_TRIANGLES, marching_cubes_tri_indices.size() * 3,
...
void MarchingCubes::PaintTetra(const GridValue &a, const GridValue &b,
                               const GridValue &c, const GridValue &d,
                               double isosurface) {
  // figure out how many of the vertices are on each side of the isosurface
  int count = 0;
  if (a.value > isosurface)
    count++;
  if (b.value > isosurface)
    count++;
  if (c.value > isosurface)
    count++;
  if (d.value > isosurface)
    count++;
  // handle each case
  if (count == 4) {
    ...
    if (d.value <= isosurface)
      PaintTetraHelper3(a, b, c, d, isosurface);
    else if (c.value <= isosurface)
      PaintTetraHelper3(a, d, b, c, isosurface);
    else if (b.value <= isosurface)
      PaintTetraHelper3(a, c, d, b, isosurface);
    else if (a.value <= isosurface)
      PaintTetraHelper3(b, d, c, a, isosurface);
    else
      assert(0);
  } else if (count == 2) {
    if (a.value > isosurface && b.value > isosurface)
      PaintTetraHelper2(a, b, c, d, isosurface);
    else if (a.value > isosurface && c.value > isosurface)
      PaintTetraHelper2(a, c, d, b, isosurface);
    else if (a.value > isosurface && d.value > isosurface)
      PaintTetraHelper2(a, d, b, c, isosurface);
    else if (b.value > isosurface && c.value > isosurface)
      PaintTetraHelper2(b, c, a, d, isosurface);
    else if (b.value > isosurface && d.value > isosurface)
      PaintTetraHelper2(b, d, c, a, isosurface);
    else if (c.value > isosurface && d.value > isosurface)
      PaintTetraHelper2(c, d, a, b, isosurface);
    else
      assert(0);
  } else if (count == 1) {
    if (a.value > isosurface)
      PaintTetraHelper1(a, b, c, d, isosurface);
    else if (b.value > isosurface)
      PaintTetraHelper1(b, c, a, d, isosurface);
    else if (c.value > isosurface)
      PaintTetraHelper1(c, a, b, d, isosurface);
    else if (d.value > isosurface)
      PaintTetraHelper1(d, c, b, a, isosurface);
    else
      assert(0);
  } else {
    // count == 0, do nothing
...
void MarchingCubes::PaintTetraHelper4(const GridValue &a, const GridValue &b,
                                      const GridValue &c, const GridValue &d,
                                      double isosurface) {
  assert(a.value > isosurface && b.value > isosurface && c.value > isosurface &&
         d.value > isosurface);
...
void MarchingCubes::PaintTetraHelper3(const GridValue &a, const GridValue &b,
                                      const GridValue &c, const GridValue &d,
                                      double isosurface) {
  assert(a.value > isosurface && b.value > isosurface && c.value > isosurface &&
         d.value <= isosurface);
...
void MarchingCubes::PaintTetraHelper2(const GridValue &a, const GridValue &b,
                                      const GridValue &c, const GridValue &d,
                                      double isosurface) {
  assert(a.value > isosurface && b.value > isosurface &&
         c.value <= isosurface && d.value <= isosurface);
...
void MarchingCubes::PaintTetraHelper1(const GridValue &a, const GridValue &b,
                                      const GridValue &c, const GridValue &d,
                                      double isosurface) {
  assert(a.value > isosurface && b.value <= isosurface &&
         c.value <= isosurface && d.value <= isosurface);
...
void MarchingCubes::drawIfBoundary(const glm::vec3 &p1, const glm::vec3 &p2,
                                   const glm::vec3 &p3) {
  if ((p1.x <= 0.1 * dx && p2.x <= 0.1 * dx && p3.x <= 0.1 * dx) ||
      (p1.y <= 0.1 * dy && p2.y <= 0.1 * dy && p3.y <= 0.1 * dy) ||
      (p1.z <= 0.1 * dz && p2.z <= 0.1 * dz && p3.z <= 0.1 * dz) ||
      (p1.x >= dx * (nx - 1.1) && p2.x >= dx * (nx - 1.1) &&
       p3.x >= dx * (nx - 1.1)) ||
      (p1.y >= dy * (ny - 1.1) && p2.y >= dy * (ny - 1.1) &&
       p3.y >= dy * (ny - 1.1)) ||
      (p1.z >= dz * (nz - 1.1) && p2.z >= dz * (nz - 1.1) &&
       p3.z >= dz * (nz - 1.1))) {
