...
void Cloth::AddWireFrameTriangle(
    const glm::vec3 &apos, const glm::vec3 &bpos, const glm::vec3 &cpos,
    const glm::vec3 &anormal, const glm::vec3 &bnormal,
    const glm::vec3 &cnormal, const glm::vec3 &abcolor,
    const glm::vec3 &bccolor, const glm::vec3 &cacolor) {
...
glm::vec3 super_elastic_color(const ClothParticle &a, const ClothParticle &b,
                              double correction) {
...
void Cloth::setupVBOs() {
  ...
  for (int i = 0; i < nx - 1; i++) {
    for (int j = 0; j < ny - 1; j++) {
      ...
      glm::vec3 ab_color =
          super_elastic_color(a, b, provot_structural_correction);
      glm::vec3 bc_color =
          super_elastic_color(b, c, provot_structural_correction);
      glm::vec3 cd_color =
          super_elastic_color(c, d, provot_structural_correction);
      glm::vec3 da_color =
          super_elastic_color(d, a, provot_structural_correction);
      ...
      AddWireFrameTriangle(a_pos, b_pos, x_pos, a_normal, b_normal, x_normal,
                           ab_color, bd_color, ac_color);
      AddWireFrameTriangle(b_pos, c_pos, x_pos, b_normal, c_normal, x_normal,
                           bc_color, ac_color, bd_color);
      AddWireFrameTriangle(c_pos, d_pos, x_pos, c_normal, d_normal, x_normal,
                           cd_color, bd_color, ac_color);
      AddWireFrameTriangle(d_pos, a_pos, x_pos, d_normal, a_normal, x_normal,
  ...
  for (int i = 0; i < nx; i++) {
    for (int j = 0; j < ny; j++) {
      ...
      if (args->velocity) {
        addEdgeGeometry(cloth_velocity_verts, cloth_velocity_tri_indices, pos,
                        pos + dt * 100 * vel, glm::vec3(1, 0, 0),
                        glm::vec3(1, 0, 0), thickness, thickness);
      }
  ...
  if (cloth_verts.size() > 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * cloth_verts.size(),
                 &cloth_verts[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cloth_tri_indices_VBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(VBOIndexedTri) * cloth_tri_indices.size(),
                 &cloth_tri_indices[0], GL_STATIC_DRAW);
  }
  if (cloth_velocity_verts.size() > 0) {
    glBindBuffer(GL_ARRAY_BUFFER, cloth_velocity_verts_VBO);
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * cloth_velocity_verts.size(),
                 &cloth_velocity_verts[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cloth_velocity_tri_indices_VBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(VBOIndexedTri) * cloth_velocity_tri_indices.size(),
                 &cloth_velocity_tri_indices[0], GL_STATIC_DRAW);
  }
  if (cloth_force_verts.size() > 0) {
    glBindBuffer(GL_ARRAY_BUFFER, cloth_force_verts_VBO);
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * cloth_force_verts.size(),
                 &cloth_force_verts[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cloth_force_tri_indices_VBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(VBOIndexedTri) * cloth_force_tri_indices.size(),
                 &cloth_force_tri_indices[0], GL_STATIC_DRAW);
  }
...
void Cloth::drawVBOs() {
  ...
  if (args->particles && cloth_verts.size() > 0) {
    ...
    // There are 8 *extra* points for every grid cell, so set the stride to be 9
    // vertices....
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE,
                          sizeof(VBOPosNormalColor) * 9, 0);
  ...
  if (args->surface && cloth_tri_indices.size() > 0) {
    ...
    glDrawElements(GL_TRIANGLES, cloth_tri_indices.size() * 3, GL_UNSIGNED_INT,
                   BUFFER_OFFSET(0));
    glDisableVertexAttribArray(0);
  ...
  if (args->velocity && cloth_velocity_tri_indices.size() > 0) {
    ...
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(glm::vec3),
                          (void *)(sizeof(glm::vec3) * 2));
    glDrawElements(GL_TRIANGLES, cloth_velocity_tri_indices.size() * 3,
