...
public:
  ...
  BoundingBox getBoundingBox() const {
    return BoundingBox(glm::vec3(0, 0, 0),
                       glm::vec3(nx * dx, ny * dy, nz * dz));
  }
...
private:
  ...
  double getPressure(int i, int j, int k) const {
    return getCell(i, j, k)->getPressure();
  }
  double ComputeDivergence(int i, int j, int k) const;
  double ComputeDP(double divergence) const;
  // velocity accessors
  double get_u_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_u_plus();
  }
  double get_v_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_v_plus();
  }
  double get_w_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_w_plus();
  }
  double get_new_u_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_new_u_plus();
  }
  double get_new_v_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_new_v_plus();
  }
  double get_new_w_plus(int i, int j, int k) const {
    return getCell(i, j, k)->get_new_w_plus();
  }
  double get_u_avg(int i, int j, int k) const {
    return 0.5 * (get_u_plus(i - 1, j, k) + get_u_plus(i, j, k));
  }
  double get_v_avg(int i, int j, int k) const {
    return 0.5 * (get_v_plus(i, j - 1, k) + get_v_plus(i, j, k));
  }
  double get_w_avg(int i, int j, int k) const {
    return 0.5 * (get_w_plus(i, j, k - 1) + get_w_plus(i, j, k));
  }
  double get_uv_plus(int i, int j, int k) const {
    return 0.5 * (get_u_plus(i, j, k) + get_u_plus(i, j + 1, k)) * 0.5 *
           (get_v_plus(i, j, k) + get_v_plus(i + 1, j, k));
  }
  double get_uw_plus(int i, int j, int k) const {
    return 0.5 * (get_u_plus(i, j, k) + get_u_plus(i, j, k + 1)) * 0.5 *
           (get_w_plus(i, j, k) + get_w_plus(i + 1, j, k));
  }
  double get_vw_plus(int i, int j, int k) const {
    return 0.5 * (get_v_plus(i, j, k) + get_v_plus(i, j, k + 1)) * 0.5 *
           (get_w_plus(i, j, k) + get_w_plus(i, j + 1, k));
  }
  // velocity modifiers
  void set_new_u_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->set_new_u_plus(f);
  }
  void set_new_v_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->set_new_v_plus(f);
  }
  void set_new_w_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->set_new_w_plus(f);
  }
  void adjust_new_u_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->adjust_new_u_plus(f);
  }
  void adjust_new_v_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->adjust_new_v_plus(f);
  }
  void adjust_new_w_plus(int i, int j, int k, double f) {
    getCell(i, j, k)->adjust_new_w_plus(f);
  }
  ...
  void GenerateParticles(const std::string &shape,
...
void setupCubeVBO(const glm::vec3 pts[8], const glm::vec3 &color,
