...
void Fluid::setupVBOs() {
  ...
  if (args->velocity) {
    if (args->dense_velocity == 0) {
      ...
      for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
          for (int k = 0; k < nz; k++) {
            glm::vec3 cell_center((i + 0.5) * dx, (j + 0.5) * dy,
                                  (k + 0.5) * dz);
            glm::vec3 direction(get_u_avg(i, j, k), get_v_avg(i, j, k),
                                get_w_avg(i, j, k));
            glm::vec3 pt2 =
                cell_center + float(100 * args->timestep) * direction;
            addEdgeGeometry(fluid_velocity_verts, fluid_velocity_tri_indices,
                            cell_center, pt2, glm::vec3(1, 0, 0),
                            glm::vec3(1, 0, 0), thickness, thickness * 0.1);
          }
        }
      }
      ...
      for (double x = 0; x <= (nx + 0.01) * dx; x += 0.25 * dx) {
        for (double y = 0; y <= (ny + 0.01) * dy; y += 0.25 * dy) {
          ...
          addEdgeGeometry(fluid_velocity_verts, fluid_velocity_tri_indices, pt1,
                          pt2, glm::vec3(1, 0, 0), glm::vec3(1, 0, 0),
  ...
  // render stubby triangles to visualize the u, v, and w velocities between
  // cell faces
  // =====================================================================================
  ...
  if (args->pressure) {
    for (int i = 0; i < nx; i++) {
      for (int j = 0; j < ny; j++) {
        for (int k = 0; k < nz; k++) {
          glm::vec3 pts[8] = {
              glm::vec3((i + 0.1) * dx, (j + 0.1) * dy, (k + 0.1) * dz),
  ...
  if (fluid_particles.size() > 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * fluid_particles.size(),
                 &fluid_particles[0], GL_STATIC_DRAW);
  }
  ...
  if (fluid_velocity_verts.size() > 0 &&
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * fluid_velocity_verts.size(),
                 &fluid_velocity_verts[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, fluid_velocity_tri_indices_VBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(VBOIndexedTri) * fluid_velocity_tri_indices.size(),
                 &fluid_velocity_tri_indices[0], GL_STATIC_DRAW);
  }
  ...
  if (fluid_facevelocity_verts.size() > 0 &&
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * fluid_facevelocity_verts.size(),
                 &fluid_facevelocity_verts[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, fluid_facevelocity_tri_indices_VBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(VBOIndexedTri) * fluid_facevelocity_tri_indices.size(),
                 &fluid_facevelocity_tri_indices[0], GL_STATIC_DRAW);
  }
  ...
  if (fluid_cell_type_vis.size() > 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(VBOPosNormalColor) * fluid_cell_type_vis.size(),
                 &fluid_cell_type_vis[0], GL_STATIC_DRAW);
  }
  ...
  for (int i = 0; i <= nx; i++) {
    for (int j = 0; j <= ny; j++) {
      for (int k = 0; k <= nz; k++) {
        marchingCubes->set(
            i, j, k,
            interpolateIsovalue(glm::vec3((i - 0.5), (j - 0.5), (k - 0.5))));
      }
    }
  }
...
void Fluid::drawVBOs() {
  ...
  if (args->velocity && fluid_velocity_tri_indices.size() > 0) {
    ...
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(glm::vec3),
                          (void *)(sizeof(glm::vec3) * 2));
    glDrawElements(GL_TRIANGLES, fluid_velocity_tri_indices.size() * 3,
  ...
  if (args->face_velocity && fluid_facevelocity_tri_indices.size() > 0) {
    ...
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(glm::vec3),
                          (void *)(sizeof(glm::vec3) * 2));
    glDrawElements(GL_TRIANGLES, fluid_facevelocity_tri_indices.size() * 3,
                   GL_UNSIGNED_INT, BUFFER_OFFSET(0));
    glDisableVertexAttribArray(0);
    glDisableVertexAttribArray(1);
    glDisableVertexAttribArray(2);
...
double Fluid::getIsovalue(int i, int j, int k) const {
  ...
  if (c->getStatus() == CELL_SURFACE)
    return 0.5 + c->numParticles() / double(density);
...
double Fluid::interpolateIsovalue(const glm::vec3 &v) const {
  ...
  double answer =
      triInterpolate(x_frac, y_frac, z_frac, a, b, c, d, e, f, g, h);
  return answer;
}
